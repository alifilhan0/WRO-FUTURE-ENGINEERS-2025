...
#define SERVO_PULSE_GPIO             17        // GPIO connects to the PWM signal line
...
{
    if(angle > 16) angle = 16; //20
    if(angle < -16) angle =-16; //-20
    return ((angle + CALIBRATION) - SERVO_MIN_DEGREE) * (SERVO_MAX_PULSEWIDTH_US - SERVO_MIN_PULSEWIDTH_US) / (SERVO_MAX_DEGREE - SERVO_MIN_DEGREE) + SERVO_MIN_PULSEWIDTH_US;
}

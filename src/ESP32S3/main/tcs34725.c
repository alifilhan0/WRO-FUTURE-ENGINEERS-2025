...
static esp_err_t Done(ESP32_TCS34725* TCS, i2c_cmd_handle_t i)
{
    i2c_master_stop(i);
    int err = i2c_master_cmd_begin(TCS->i2c_port, i, 10 / portTICK_PERIOD_MS);
    if (err)
        return err;
...
static i2c_cmd_handle_t Read(ESP32_TCS34725* TCS, uint16_t reg)
{                               // Set up for read
    i2c_cmd_handle_t i = i2c_cmd_link_create();
    i2c_master_start(i);
    i2c_master_write_byte(i, (TCS34725_ADDRESS << 1), 1);
    i2c_master_write_byte(i, reg >> 8, 1);
    i2c_master_write_byte(i, reg, 1);
    Done(TCS, i);
    i = i2c_cmd_link_create();
    i2c_master_start(i);
    i2c_master_write_byte(i, (TCS34725_ADDRESS << 1) + 1, 1);
    return i;
}

static i2c_cmd_handle_t Write(ESP32_TCS34725* TCS, uint16_t reg)
{                               // Set up for write
    i2c_cmd_handle_t i = i2c_cmd_link_create();
    i2c_master_start(i);
    i2c_master_write_byte(i, (TCS34725_ADDRESS << 1), 1);
...
static void write8(ESP32_TCS34725* TCS, uint8_t reg, uint32_t value) {
  ...
  i2c_cmd_handle_t i = Write(TCS, reg);
  i2c_master_write_byte(i, value, 1);
  Done(TCS, i);
...
static uint8_t read8(ESP32_TCS34725* TCS, uint8_t reg) {
  ...
  i2c_master_start(i);
  i2c_master_write_byte(i, (TCS34725_ADDRESS << 1) + 1, 1);
  i2c_master_read_byte(i, buffer + 0, I2C_MASTER_LAST_NACK);
  Done(TCS, i);
  /*i2c_master_write_to_device(TCS->i2c_port, TCS34725_ADDRESS, buffer, 1, 1000 / portTICK_PERIOD_MS);
  i2c_master_read_from_device(TCS->i2c_port, TCS34725_ADDRESS, buffer, 1, 1000 / portTICK_PERIOD_MS);    */
...
uint8_t buf[2] = { };
  i2c_cmd_handle_t i = Read(TCS, reg);
  ...
  /*
  i2c_master_write_to_device(TCS->i2c_port, TCS34725_ADDRESS, buffer, 1, 1000 / portTICK_PERIOD_MS));
  i2c_master_read_from_device(TCS->i2c_port, TCS34725_ADDRESS, buffer, 2, 1000 / portTICK_PERIOD_MS));
 ...
 */
esp_err_t TCS_init(ESP32_TCS34725 *TCS, uint8_t i2c_port, uint8_t sda, uint8_t scl) {
    if(!TCS->_tcs34725Initialised){
        ...
        if (i2c_param_config(TCS->i2c_port, &config))
        {                            // Config failed
            i2c_driver_delete(TCS->i2c_port);
            return -1;
        }
        i2c_set_timeout(TCS->i2c_port, 100000);        // Clock stretching
        i2c_filter_enable(TCS->i2c_port, 5);

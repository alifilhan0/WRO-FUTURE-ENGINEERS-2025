/*****************************************************************************
* Waveshare AS7341 Library ported by Alif Ilhan
*
******************************************************************************/
#include "esp_timer.h"
#include "esp_log.h"
...
static esp_err_t
Done (i2c_cmd_handle_t i)
{
   i2c_master_stop (i);
   esp_err_t err = i2c_master_cmd_begin (0, i, 80000);
...
{                               // Set up for read
   ...
   i2c_master_start (i);
   i2c_master_write_byte (i, (0x39 << 1), 1);
   i2c_master_write_byte (i, reg, 1);
   Done (i);
   i = i2c_cmd_link_create ();
   i2c_master_start (i);
   i2c_master_write_byte (i, (0x39 << 1) + 1, 1);
   return i;
}
...
static UBYTE AS7341_Read_Byte(UBYTE reg)
{
...
static void AS7341_Write_Byte(UBYTE reg, UBYTE val)
{
...
{
	//Put the i2c init sequence here
